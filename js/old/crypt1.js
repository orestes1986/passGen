function hsore1(str, noChar){
   // console.log("Hey HSORE " + str);
    ...
    while (str[k]){
        var l = 0;
        while (str[l]){
            crpt[0] += Math.abs((Math.pow(str.charCodeAt(l), (1/2))) - (Math.pow(str.charCodeAt(k), (1/9))));
            crpt[1] += Math.abs((Math.pow(str.charCodeAt(l), (1/3))) - (Math.pow(str.charCodeAt(k), (1/5))));
            crpt[2] += Math.abs((Math.pow(str.charCodeAt(l), (1/9))) - (Math.pow(str.charCodeAt(k), (1/3))));
            crpt[3] += Math.abs((Math.pow(str.charCodeAt(l), (1/5))) - (Math.pow(str.charCodeAt(k), (1/2))));
            l++;
        }
        k++;
    }
...
function func1(us, prov, times, leng, notWant) {
    if (us === ""){
        us = "User's insignia";
    }
    if (prov === ""){
   ...
   /* var strum = "γειά σ세요" + us + " سلامου!%$#μ" + prov + "ε ハーワールド 안녕하 جهان ਸੰਸਾ%_&*) Κόσਰ హలో వ" + times +"రల్డ్ โลก העלא וועלט העלא וועלט لو دنیا ஹலோ உலகம் 你好"+ leng +"世界";*/

    console.log("Hey Func " + strum);
    console.log("times : " + times);
    for (var j = 0; j <= times; j++){
        strum = hsore1(strum, notWant);
    }
    var shtring = "";
    ...
    /*return shtring;*/
    document.getElementById("outText").innerHTML = shtring;
    return shtring;
}
